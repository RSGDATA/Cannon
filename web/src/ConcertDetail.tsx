...
type PScore = { live_count: number; live_fresh_pct: number | null; heard_count: number }
...
export default function ConcertDetail({ id, session, onBack }: { id: string; session: Session | null; onBack: () => void }) {
  ...
  const [score, setScore] = useState<{ ratings: number; fresh_pct: number } | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: cs }, { data: cps }] = await Promise.all([
      supabase.from('concert_score').select('ratings,fresh_pct').eq('concert_id', id).maybeSingle(),
      supabase.from('concert_piece_score').select('work_id,live_count,live_fresh_pct,heard_count').eq('concert_id', id),
    ])
    setScore((cs as { ratings: number; fresh_pct: number } | null) ?? null)
